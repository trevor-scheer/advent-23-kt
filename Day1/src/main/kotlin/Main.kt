...
fun firstHalf() {
    ...
    for (str in arr) {
        val matchResults = regex.findAll(str)
        val first = matchResults.first().value
        val last = matchResults.last().value
        val asNum = (first + last).toInt()
        nums += asNum
    }

    val answer = nums.reduce { acc, num -> acc + num }
    println(answer)
}

fun secondHalf() {
    val numsAsWords = "one,two,three,four,five,six,seven,eight,nine".split(',')
    val forwardRegEx = Regex(numsAsWords.joinToString("|", "[0-9]|"))
    val reverseRegEx = Regex(numsAsWords.joinToString("|", "[0-9]|") { word -> word.reversed() })
    val wordsToNumsMap = numsAsWords.mapIndexed { index, s -> s to (index + 1).toString()}.toMap()
    var nums = emptyArray<Int>()

    for (str in arr) {
        var firstDigit = forwardRegEx.find(str)?.value
        var lastDigit = reverseRegEx.find(str.reversed())?.value?.reversed()

        firstDigit = wordsToNumsMap.getOrDefault(firstDigit, firstDigit)
        lastDigit = wordsToNumsMap.getOrDefault(lastDigit, lastDigit)

        nums += (firstDigit + lastDigit).toInt()
    }
